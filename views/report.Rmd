...
$u_r\text{(SamplePreparation)} = \sqrt{\sum{[u_r(\text{Equipment})_{\text{(Vol,Tol)}}^2 \times N(\text{Equipment})_{\text{(Cap,Tol)}}}]}$
...
If data is uploaded for all the uncertainty components; Calibration Curve, Method Precision, Calibration Standard and Sample Preparation, relative standard uncertainty is computed for each uncertainty component and are combined to obtain the Combined Uncertainty of the analytical process.
...
- \(k_{{\large\nu}_{\text{eff}}, {\small CI\%}}\) is the coverage factor based on effective degrees of freedom and specified confidence level percentage.
